...
// const addBooks = async (req, res) => {
//   const { namaBuku, penerbit, pengarang, tahunTerbit, tempatTerbit, isbn, jmlhhlmn, abstrak, category } = req.body;
//   const pdfUrl = req.files && req.files['pdfUrl'] ? req.files['pdfUrl'][0].id : null;
//   const imageUrl = req.files && req.files['imageUrl'] ? req.files['imageUrl'][0].id : null;
...
//     await gfs.remove({ _id: book.pdfUrl, root: 'uploads' });
//     if (book.imageUrl) {
//       await gfs.remove({ _id: book.imageUrl, root: 'uploads' });
//     }
...
//   const pdfUrlPath = path.join(__dirname, ../${pdfUrl});
//   res.download(pdfUrlPath);
// };

//bisa auto download
// Fungsi downloadFile yang diperbaiki untuk menggunakan GridFS
...
//       } else {
//         res.status(400).json({ message: "Not a PDF file" });
//       }
//     });
//   } catch (err) {
//     console.error(err);
